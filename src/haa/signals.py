...
def latest_actionable_signal(
    ...
    allocation silently.
    """
    completed_through = last_completed_month_end(as_of)
    assets = tuple(required_assets)
    if decisions.empty:
        return SignalStatus(None, "No strategy decisions exist yet; at least 12 prior monthly observations are required.", completed_through)
    if any(asset not in monthly_prices.columns for asset in assets):
        return SignalStatus(None, "Required asset data is unavailable.", completed_through)
    if completed_through not in monthly_prices.index:
        return SignalStatus(None, f"No complete monthly price row is available for {completed_through.date()}.", completed_through)
