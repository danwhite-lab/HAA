...
The app downloads Yahoo Finance history automatically. For independent validation, upload one CSV per asset. A CSV needs `Date` and `Adj Close` (preferred) or `Close`; the upload replaces that asset's full history rather than silently filling gaps.
...
This is a standard Streamlit project. Push this repository to GitHub, then create an app at [Streamlit Community Cloud](https://share.streamlit.io/) and point it to `app.py`. Install dependencies from `pyproject.toml` (or use `pip install -e .`).
