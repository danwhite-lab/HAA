...
def test_13612w_matches_manual_example():
    ...
    expected = (12 * (120 / 110 - 1) + 4 * (120 / 110 - 1) + 2 * (120 / 106 - 1) + (120 / 100 - 1)) / 4
    assert momentum_13612w(prices).iloc[-1] == pytest_approx(expected)
...
def test_momentum_never_uses_future_prices():
    ...
    baseline = momentum_13612w(prices).iloc[12]
    prices.iloc[13:] = 1_000_000
    assert momentum_13612w(prices).iloc[12] == baseline
